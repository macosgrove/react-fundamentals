...
React requires `onSubmit={handleSubmit}` where regular javascript has `onsubmit="handleSubmit()"`

.toLowerCase is a FUNCTION on string, not an attribute, so value.toLowerCase() with brackets
